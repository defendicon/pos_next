			...
			<div class="py-4">
				...
				<div v-else-if="mode === 'uom' && options.length > 0">
					...
					<div class="mb-4">
						...
						/>
						<!-- Stock Warning -->
						<div v-if="showStockWarning" class="mt-2 flex items-center gap-1.5 text-orange-600">
							<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
							</svg>
							<span class="text-xs font-medium">{{ __('Requested quantity exceeds available stock') }}</span>
						</div>
					</div>
...
const showStockWarning = computed(() => {
	if (props.mode !== 'uom' || !selectedOption.value || !props.item) return false

	// Calculate requested quantity in stock UOM
	const factor = selectedOption.value.conversion_factor || 1
	const requestedQty = (quantity.value || 0) * factor

	// Get available stock
	const stock = stockStore.getDisplayStock(props.item.item_code)
