...
export const usePOSCartStore = defineStore("posCart", () => {
	...
	function parseOfferResponse(response, fallbackRules = []) {
		...
		return {
			...
			appliedRules:
				Array.isArray(payload.applied_pricing_rules) &&
				payload.applied_pricing_rules.length
	...
	async function reapplyOffer(currentProfile) {
		...
		try {
			...
			if (invalidOffers.length > 0) {
				...
				showWarning(
					`Offer removed: ${offerNames}. Cart no longer meets requirements.`,
				)
